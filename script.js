...
// code to get a new index everyday of the year
const offsetFromDate = new Date(2022, 0, 1);
const msOffset = Date.now() - offsetFromDate;
const coversionRateofMsToDays = 86_400_000;
const daysOffset = msOffset / coversionRateofMsToDays;
...
const handleKeyPress = (e) => {
  ...
  if (e.key.match(/^[a-z]$/)) {
    // added double quotes to match param (reason of error)
    pressKey(e.key);
    return;
  }
};
...
const deleteKey = () => {
  const activeTiles = getActiveTiles();
  let lastActiveTile = activeTiles[activeTiles.length - 1];
...
const danceTiles = (tiles) => {
  tiles.forEach((tile, index) => {
    ...
    }, (index * DANCE_ANIMATION_DURATION) / 2);
  });
};
